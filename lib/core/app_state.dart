...
class AppState with ChangeNotifier {
  ...
  Future<int> addSampleMarking({
    ...
    if (order != null && order.firebaseId == null) {
      print('Order ${order.clientName} has no firebaseId, uploading to Firebase first...');
      try {
        await _syncService.uploadOrder(order);
        // Reload the order to get the updated firebaseId
        final updatedOrder = await _orderDao.getOrderById(orderId);
        if (updatedOrder?.firebaseId != null) {
          print('Order uploaded successfully, firebaseId: ${updatedOrder!.firebaseId}');
        } else {
          print('Warning: Order upload completed but firebaseId still null');
        }
      } catch (e) {
        print('Error uploading order to Firebase: $e');
