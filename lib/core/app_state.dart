...
class AppState with ChangeNotifier {
  ...
  Future<int> addSampleMarking({
    ...
    // CRITICAL: Ensure the order has a firebaseId before adding samples
    // This is necessary for the web admin to correctly count samples
    var currentOrder = await _orderDao.getOrderById(orderId);
    if (currentOrder != null && currentOrder.firebaseId == null) {
      print('Order ${currentOrder.clientName} has no firebaseId, uploading to Firebase first...');
      try {
        await _syncService.uploadOrder(currentOrder);

        // Wait briefly for database write to complete
        await Future.delayed(Duration(milliseconds: 100));

        // Reload the order to get the updated firebaseId
        currentOrder = await _orderDao.getOrderById(orderId);
        if (currentOrder?.firebaseId != null) {
          print('Order uploaded successfully, firebaseId: ${currentOrder!.firebaseId}');
        } else {
          print('ERROR: Order upload completed but firebaseId still null!');
          print('This will cause samples to not appear in web admin.');
        }
      } catch (e) {
        print('Error uploading order to Firebase: $e');
        // Continue anyway - sample will be saved locally
      }
    } else if (currentOrder?.firebaseId != null) {
      print('Order ${currentOrder!.clientName} already has firebaseId: ${currentOrder.firebaseId}');
    }

    // Verify order has firebaseId before proceeding
    if (currentOrder != null && currentOrder.firebaseId == null) {
      print('WARNING: Proceeding to add sample but order has no firebaseId!');
      print('Sample may not appear in web admin until order is properly synced.');
    }
    ...
    if (savedSample != null) {
      ...
      try {
        print('Uploading sample for order with firebaseId: ${currentOrder?.firebaseId}');
